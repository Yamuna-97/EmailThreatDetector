...
const LEGEND_ITEMS = [
  { name: 'DLP Alerts', color: '#FF1E2D' },
  { name: 'Phishing Vectors', color: '#FF5A36' },
  { name: 'Threat Intel', color: '#991B1B' },
];
...
const DEFAULT_METRICS: MetricInfo[] = [
  ...
  },
  {
    ...
    trendBaseColor: '#FF1E2D',
    trendStrokeColor: '#FF1E2D',
    delay: 0.05,
  },
  {
    id: 'ier',
...
export const IncidentReportCard: React.FC<IncidentReportCardProps> = ({
  ...
  return (
    <div className={`flex flex-col p-6 bg-[#111111] rounded-3xl border border-[#2A2A2A] shadow-sm w-full overflow-hidden transition-all ${className}`}>
      <style>{`
        :root {
          --reaviz-tick-fill: #737373;
          --reaviz-gridline-stroke: rgba(255, 30, 45, 0.08);
        }
      `}</style>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-heading text-lg sm:text-xl font-extrabold text-[#F5F5F5]">
            {title}
          </h3>
      ...
      <div className="flex flex-wrap gap-4 items-center mb-4 pt-1">
        {LEGEND_ITEMS.map((item) => (
          <div key={item.name} className="flex gap-2 items-center">
            ...
            <span className="text-[#A3A3A3] text-xs font-medium">{item.name}</span>
          </div>
        ))}
      </div>
        ...
        <AreaChart
          ...
          series={
            <AreaSeries
              ...
              colorScheme={['#FF1E2D', '#991B1B', '#FF5A36']}
            />
          }
      ...
      <div className="flex flex-col pt-6 font-body divide-y divide-[#2A2A2A]">
        {metrics.map((metric) => (
          <motion.div
            key={metric.id}
          ...
          >
            <div className="flex flex-row gap-2.5 items-center text-xs sm:text-sm font-medium text-[#A3A3A3]">
              <metric.Icon />
            ...
            <div className="flex gap-2.5 items-center shrink-0">
              <span className="font-heading font-extrabold text-base sm:text-lg text-[#F5F5F5]">
                {metric.value}
              </span>
